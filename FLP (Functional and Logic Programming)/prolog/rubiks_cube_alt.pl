...
% Stores hash of a Cube state in predicate visited
store_visited(Cube) :- term_hash(Cube, Hash), assertz(visited(Hash)).

% Checks if given Cube was already visited (if visited predicate with given hash exists)
already_visited(Cube) :- term_hash(Cube, Hash), visited(Hash).

% All possible rotations of a Cube
all_rotations_list([up_cw, up_ccw, down_cw, down_ccw, right_cw, right_ccw,
                    left_cw, left_ccw, front_cw, front_ccw, back_cw, back_ccw,
                    middle_cw, middle_ccw, equator_cw, equator_ccw, slice_cw, slice_ccw]).


% Based on given Cube generates list of Cubes with all possible rotations
% + Add rotation to given Path so it will be possible to find the way back
% Cube states that were alredy visited are omitted
% RotatedCubeList is list of tuples [Rotated Cube, Path From Given Cube to Rotated Cube]
generate_rotated_cubes(Cube, Path, RotatedCubeList) :-
    all_rotations_list(Rotations),
    findall([RotatedCube, [R|Path]],
            (
                member(R, Rotations),
                rotate_cube(R, Cube, RotatedCube),
                \+ already_visited(RotatedCube)
            ),
            RotatedCubeList
        ).


% Starts Rubiks cube solution finding
solve_cube(Cube, RotationsHistory) :- 
    retractall(visited(_)), 
    find_solution(Cube, [_|RotationsHistory]).  % [_|RotationsHistory] removes initial none


% BFS over the given Cube, result is a sequence of moves that solve the cube
find_solution([[Cube, RotationsFromCube]|_], RotationsHistory) :-
    is_cube_solved(Cube),
    reverse(RotationsFromCube, RotationsHistory).   % If the solution is found, the path has to be reversed
find_solution([[Cube, RotationsFromCube]|Queue], RotationsHistory) :-
    store_visited(Cube),                            % Save already visited Cube states
    generate_rotated_cubes(Cube, RotationsFromCube, RotatedCubeList),
    append(Queue, RotatedCubeList, NewQueue),
    find_solution(NewQueue, RotationsHistory).
...
start :-
    ...
    solve_cube([[InputCube, [none]]], RotationsHistory), 
