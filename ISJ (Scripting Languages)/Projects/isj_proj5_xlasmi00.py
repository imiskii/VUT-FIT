...
def gen_quiz(qpool:list[tuple], *indexes:int, altcodes="ABCDEF", quiz:list[tuple]|None=None):
    """
    Creates new quiz
    ...
    qpool : list[tuple]
    *indexes : int
    altcodes : any itterable
    quiz : list[tuple]|None
    ...
    for index in indexes:
        try:
            question = qpool[index]
        except IndexError as e:
            print(f"Ignoring index {index} - {e}")
        else:
            answers = []
            alt_choices = [str(choice) for choice in altcodes]
            for answer in zip(alt_choices, question[1]):
                result = answer[0] + ": " + answer[1]
                answers.append(result)
            toAdd = (question[0], answers)
            quiz.append(toAdd)
